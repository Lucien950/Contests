...
for case in range(5):
	...
	itemPrices.sort()
	itemPrices.reverse()
	coupons = Counter(coupons)
	coupons["BOGO"] = min(coupons["BOGO"], 1)
	explore = [(0, Decimal(0), copy.deepcopy(coupons))]
	bestPrice = Decimal('Infinity')
	while explore:
		i, price, nowCoupon = explore.pop(0)
		if i == len(itemPrices):
		...
		if canMultiply:
			...
			nowCoupon[percent] -= 1
			explore.append((i+1, price + cock, copy.deepcopy(nowCoupon)))
			nowCoupon[percent] += 1
		if canCash:
		...
		if canBogo:
			...
			nowCoupon["BOGO"] -= 1
			explore.append((i+2, price + cock, copy.deepcopy(nowCoupon)))
			nowCoupon["BOGO"] += 1

		explore.append((i + 1, round(price + itemPrices[i] * Decimal(1.13), 2), copy.deepcopy(nowCoupon)))
